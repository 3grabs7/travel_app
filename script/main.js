...
document.querySelector(".search__term i").addEventListener("click", () => {
  navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    let returnObj = await apiCallLatLon(
      pos.coords.latitude,
...
checkboxes.forEach(function (box) {
  box.addEventListener("change", function (e) {
    ...
    if (e.currentTarget.name !== "sort") {
      ...
      } else if (optionsValues[1][1]) {
        attraction.checked = false;
      }
    }
    optionsValues = [];
...
document.getElementById("searchbutton").addEventListener("click", async () => {
  ...
  if (!optionsValues[1][1] && !optionsValues[0][1]) {
    attractions.style.display = "block";
    weather.style.display = "flex";
  }
  if (optionsValues[2][1]) {
    sorted = true;
  } else {
    sorted = false;
  }
...
function updateWeather(obj) {
  let weatherForm = document.querySelectorAll(
    ".results__weather__details__item p"
  );
  let weather = [
  ...
  setTimeout(() => {
    document.querySelector(
      ".results__weather__top h1"
    ).innerHTML = `Weather <br> <span style="font-size:60px">${obj.name}</span>`;
    document.getElementById(
      "weatherimg"
    ).src = `http://openweathermap.org/img/wn/${obj.weather[0].icon.replace(
      "n",
      "d"
    )}@2x.png`;
    weatherForm.forEach((o, i) => {
      o.innerHTML = weather[i];
    });
  }, 750);
}
      ...
      if (categories[i].name === checkbox.name) {
        if (categories[i].checked) {
          ...
          checkbox.nextElementSibling.style =
            "background-color:none;color:rgb(151, 117, 252)";
          ...
          checkbox.nextElementSibling.style =
            "background-color:rgb(92, 145, 243);color:black";
          checkbox.nextElementSibling.addEventListener("mouseover", (e) => {
...
async function attraction(city) {
  ...
  if (categories.filter((c) => c.checked).length === 0) {
    ...
    let venues = {
      ...
      category: json.response.venues
        .map((v) => v.categories[0])
        .map((i) => i?.name ?? "Unknown"),
      address: json.response.venues.map((v) => v.location.address),
      icon: json.response.venues
        .map((v) => v.categories[0])
        .map((i) => i?.icon ?? "Unknown"),
    };
    // Update results based on the object we created from response
    loadAttractions(venues);
    // Else -> make one API call per category checked, 12 results per call
  } else {
    for (let i = 0; i < categories.length; i++) {
      if (categories[i].checked) {
        // Update query with search term
        let query = `near=${city.replace(
          " ",
          "%20"
        )}&limit=12&sortByPopularity=1`;
        // Add query for category
        let category = `&categoryId=${categories[i].id}`;
        // Send request
        let response = await fetch(
          `${attractionURL}${query}${category}${token}`
        );
        // Await response and convert to json
        let json = await response.json();
...
function clearAttractions() {
  Array.from(
    document.querySelector(".results__attractions__boxes").childNodes
  ).forEach((e) => e.remove());
}
...
function loadAttractions(obj) {
  ...
  for (let i = 0; i < obj.name.length; i++) {
    ...
    if (typeof obj.icon[i] != "string") {
      ...
      // Icon not found
      icon.src =
        "https://www.freeiconspng.com/uploads/exclamation-point-error-simple-black-background-pictures-18.png";
...
window.addEventListener("scroll", () => {
  ...
  let boxes =
    document.querySelectorAll(".results__attractions__boxes__box") ?? undefined;
  if (boxes != undefined) {
    Array.from(boxes).forEach((b, i) => {
      if (scrollpos > b.offsetTop - wh - 20 && b.style.animation === "") {
        b.style.animation = `enterattraction 0.7s ${
          (i % 3) / 10
        }s linear 1 forwards`;
      }
    });
